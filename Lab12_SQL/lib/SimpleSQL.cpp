...
simple_sql::Table simple_sql::Database::ParseRequest(const std::string& str) {
    ...
    std::replace_if(s.begin(), s.end(), [&](char c) { return delimiter.find(c) != std::string::npos; }, ' ');
    ...
    if (word == "create") {
        ...
        while (ss_skip >> word) {
            ...
            if (check == "WHERE" || check == "JOIN" ||
                check == "RIGHT" || check == "LEFT" || check == "INNER") break;
...
data_types getValue(size_t type, std::string& str_value) {
    bool is_null = false;
    std::string check = str_value;
    std::transform(check.begin(), check.end(), check.begin(), ::tolower);
    if (check == "null" || check.empty()) is_null = true;
...
std::string to_string(const data_types& value) {
    std::string text;
...
void simple_sql::Database::ParseCreateLine(std::vector<std::string>& line, simple_sql::Table& table) {
    ...
    if (check_word == "foreign") {
        // FOREIGN KEY col_name REFERENCES table(col_name);
        check_word = line [1];
        std::transform(check_word.begin(), check_word.end(), check_word.begin(), ::toupper);
        if (check_word != "KEY") throw std::runtime_error("Unknown command FOREIGN " + check_word);
        check_word = line[3];
        std::transform(check_word.begin(), check_word.end(), check_word.begin(), ::toupper);
        if (check_word != "REFERENCES") throw std::runtime_error("Unknown command FOREIGN KEY " + check_word);
    ...
    if (check_word == "bool") {
        ...
        table.m_index_varsize.insert({table.column_type.size() - 1,
                                      std::stoi(line[2], nullptr, 10)});
    }
...
simple_sql::Table simple_sql::Database::CreateTableRequest(std::stringstream& ss) {
    ...
    while (ss >> word) {
        if (word == "," || word.ends_with(',')) {
            ...
            if (!word.empty()) word_line.emplace_back(word);
            ParseCreateLine(word_line, table);
            word_line.clear();
        } else {
        ...
        if (step == 0) {
            ...
            if (value_right.ends_with('\"') || (!value_right.starts_with('\"') && !value_right.empty())) {
                if (!table_name_1.empty()) {
                    ...
                    if (!has_table) throw std::runtime_error("Unknown "+ table_name_1 + "." + column_label_1 + " in WHERE clause");
        ...
        if (step == 3) {
            ...
            if (values.empty()) {
                if (value_left.empty() && value_right.empty()) {
                    ...
                    if (!has_table) throw std::runtime_error("Unknown "+ table_name_2 + "." + column_label_2 + " in WHERE clause");
                }
...
void simple_sql::Database::UpdateTableRequest(std::stringstream& ss, simple_sql::Table& table, simple_sql::WHERE& current_where) {
    // std::vector<std::string> word_line;
    bool is_label = true;
    char c;
    ...
    for (size_t i = 0; i < table.rows.size(); i++) {
        ...
        for (size_t j = i + 1; j < table.rows.size(); j++) {
            ...
            if (primary_i == primary_j) throw std::runtime_error("Update caused repeated PRIMARY KEY in table " + table.label);
...
std::ostream& simple_sql::operator<<(std::ostream& os, const simple_sql::Table& table) {
    ...
    for (auto& row: table.rows) {
        ...
        row_line = "|";
        for (size_t i = 0; i < column_num; i++) {
            std::string text = to_string(row[i]);
...
bool solve(simple_sql::WHERE& where,
    ...
    for (size_t i = 0; i < order.size(); i++) {
        if (order[i] == where.LEFT) {
            ...
            if (action == where.OR) {
                //result = result || where.conditions[order[i]].check(row);
